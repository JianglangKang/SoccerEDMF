...
#
#             args = [c1, c2, *args[1:]]
#             if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, CSPStage, C2f_Faster, C2f_DBB,}:
...
#             args = [c1, c2, *args[1:]]
#             # TODO 修改
#             if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Faster, C2f_Faster_EMA, C2f_DBB, CSPStage}:
#                 args.insert(2, n)  # number of repeats
#                 n = 1
#         elif m is AIFI:
#             args = [ch[f], *args]
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        self = super()._apply(fn)
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect,Detect_TADDH)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            m.stride = fn(m.stride)
            m.anchors = fn(m.anchors)
            m.strides = fn(m.strides)
        return self
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_TADDH):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            # TODO 修改
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Faster, C2f_Faster_EMA, C2f_DBB, CSPStage, C2f_PKIModule,C2f_RVB,C3_RVB_SE,C2f_RVB_SE,C3_RVB_EMA,C2f_RVB_EMA}:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            c2 = sum(ch[x] for x in f)
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, Detect_TADDH}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, Detect_TADDH)):
                return "detect"
